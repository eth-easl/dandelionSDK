...
// pthread_mutex_t is defined as a uint32, and the default initializer is
// 0xfff.. because that is defined elsewhere, we use setting flags to 0 to turn
// them on
#define LOCKED 0x01
...
int pthread_mutex_unlock(pthread_mutex_t *mutex) {
  ...
  if ((current & DESTROYED) == 0)
    return EINVAL;
  // fail if the lock is not locked, meaning the current (and only) thread does
  // not hold the lock
  if ((current & LOCKED) != 0)
    return EPERM;
  *mutex |= LOCKED;
