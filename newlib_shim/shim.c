...
int mkdir(const char *name, mode_t mode) {
  return process_error(dandelion_mkdir(name, mode));
}

extern int dandelion_rmdir(const char *pathname);
int rmdir(const char *pathname) {
  return process_error(dandelion_rmdir(pathname));
}
...
int link(const char *__path1, const char *__path2) {
  return process_error(dandelion_link(__path1, __path2));
}

extern int dandelion_unlink(const char *name);
int unlink(const char *__path) {
  return process_error(dandelion_unlink(__path));
}
...
extern size_t dandelion_read(int file, char *ptr, size_t len, int64_t offset,
                             char options);
#define USE_OFFSET 1
...
ssize_t pwrite(int file, const void *ptr, size_t len, off_t offset) {
  return process_error(
      dandelion_write(file, (const char *)ptr, len, offset, USE_OFFSET));
}
...
int wordexp(const char *s, wordexp_t *p, int flags) { return WRDE_NOSPACE; }

void wordfree(wordexp_t *p) { return; }