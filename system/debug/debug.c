...
int __dandelion_debug_write(int fd, const void *buffer, size_t size, ssize_t *bytes_written) {
...
int __dandelion_debug_write_all(int fd, const void *buffer, size_t size) {
	size_t written = 0;
	while (written < size) {
...
void __dandelion_debug_dump_io_buf(const char* setid, io_buffer* buf) {
	char tmp[256];
	size_t setidlen = 0;
	if (setid) {
...
int __dandelion_debug_vm_map(void *hint, size_t size, int prot, int flags,
...
int __dandelion_debug_vm_unmap(void *pointer, size_t size) {
